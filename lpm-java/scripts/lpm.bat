...
rem Liquibase Package Manager (lpm) launcher script
rem Requires Java 17 or higher

rem %~dp0 is expanded pathname of the current script under NT
set LPM_HOME="%~dp0"

rem Remove quotes around LPM_HOME
set LPM_HOME=%LPM_HOME:"=%

rem Go up one level if we're in the bin directory
if exist "%LPM_HOME%..\lib\lpm.jar" set LPM_HOME=%LPM_HOME%..\

rem Remove quotes around JAVA_HOME if set
if DEFINED JAVA_HOME if NOT "%JAVA_HOME%" == "" set JAVA_HOME=%JAVA_HOME:"=%

rem Set JAVA_HOME to local jre dir if not set and exists
if exist "%LPM_HOME%jre" if "%JAVA_HOME%"=="" (
    set JAVA_HOME="%LPM_HOME%jre"
...
set LPM_JAR=%LPM_HOME%lib\lpm.jar
if not exist "%LPM_JAR%" (
    rem Try looking in target directory (development mode)
    for /r "%LPM_HOME%" %%f in (lpm-*.jar) do (
        echo %%f | find /i "target" >nul
        if !errorlevel! equ 0 (
            echo %%f | find /i "-sources" >nul
            if !errorlevel! neq 0 (
                echo %%f | find /i "-javadoc" >nul
                if !errorlevel! neq 0 (
                    set LPM_JAR=%%f
                    goto :found_jar
                )
    ...
    echo ERROR: Cannot find lpm.jar. Please ensure lpm is properly installed. >&2
    echo Searched in: %LPM_HOME%lib\lpm.jar >&2
    exit /B 1
)
:found_jar
...
"%JAVA_PATH%" %JAVA_OPTS% -jar "%LPM_JAR%" %*
exit /b %ERRORLEVEL%
...
if !JAVA_MAJOR_VERSION! LSS 17 (
    echo ERROR: Java version is !JAVA_MAJOR_VERSION!. lpm requires Java 17 or higher. >&2
